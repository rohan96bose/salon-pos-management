<?php
            ...
            try {
                ...
                if (!empty($_POST['services'])) {
                    foreach ($_POST['services'] as $service) {
                        if (!empty($service['service_id']) && !empty($service['employee_ids'])) {
                ...
                $stmt = $pdo->prepare("INSERT INTO invoices 
                    (customer_id, created_by, discount_type, total_amount, overall_discount_value, discounted_amount, invoice_date, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
                $stmt->execute([
                    $_POST['customer_id'],
                    $_SESSION['user_id'],
                    $_POST['discount_type'],
                    $total_amount,
                    $_POST['discount_type'] !== 'none' ? $_POST['discount_value'] : 0,
                    $discounted_amount,
                    $_POST['invoice_date']
                ]);
                ...
                if (!empty($_POST['services'])) {
                    foreach ($_POST['services'] as $service) {
                        if (!empty($service['service_id']) && !empty($service['employee_ids'])) {
                            $employee_ids = is_array($service['employee_ids']) ? implode(',', $service['employee_ids']) : $service['employee_ids'];
                ...
                if (!empty($_POST['products'])) {
                    foreach ($_POST['products'] as $product) {
                        if (!empty($product['product_id'])) {
                            $employee_ids = !empty($product['employee_ids']) ? 
                                (is_array($product['employee_ids']) ? implode(',', $product['employee_ids']) : $product['employee_ids']) : 
                                null;
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT i.id, i.invoice_date, i.total_amount, i.discounted_amount, i.discount_type, i.overall_discount_value,
            ...
            <form method="POST" id="invoiceForm">
                <div class="p-4 md:p-6 space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="relative">
                            <label for="customer_search" class="block mb-2 text-sm font-medium text-gray-900">Customer</label>
                            <div class="flex gap-2 items-end">
                                <div class="flex-1 relative">
                                    ...
                                    <datalist id="customer_options">
                                        ...
                                        <option value="<?php echo htmlspecialchars($customer['name']) . ' (' . htmlspecialchars($customer['phone']) . ')'; ?>" data-id="<?php echo $customer['id']; ?>">
                                            <?php endforeach; ?>
                                    </datalist>
                                    <input type="hidden" name="customer_id" id="customer_id_hidden">
                                </div>
                                <!-- Add Customer Button (hidden by default) -->
                            <button 
    type="button" 
    id="addCustomerBtn"
    class="hidden h-[42px] px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
    onclick="window.location.href='customer-management.php'">
    Add Customer
</button>
                            </div>
                        </div>
...
<?php
    ...
    // Get employee names for each item
    foreach ($invoice_items as &$item) {
        if (!empty($item['employee_id'])) {
            $employee_ids = explode(',', $item['employee_id']);
            $employee_names = [];
            ...
            $item['employee_names'] = implode(', ', $employee_names);
        } else {
            $item['employee_names'] = 'None';
        }
    }
    unset($item);
    ?>
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <form method="POST">
                ...
                <div class="p-4 md:p-6 space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                name="invoice_date" 
                                value="<?php echo !empty($invoice_items) ? date('Y-m-d', strtotime($invoice_items[0]['invoice_date'])) : date('Y-m-d'); ?>" 
                                required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                                >
                        </div>
                        ...
                        <div id="serviceContainerEdit<?php echo $invoice['id']; ?>" class="space-y-4">
                            <?php 
                                ...
                                foreach ($invoice_items as $index => $item): 
                                    if ($item['service_id']): 
                                        $service_counter++;
                                        $employee_ids = !empty($item['employee_id']) ? explode(',', $item['employee_id']) : [];
                                ?>
                            <div class="p-4 border border-gray-200 rounded-lg bg-gray-50 service-row" id="serviceRowEdit<?php echo $invoice['id']; ?>_<?php echo $service_counter; ?>">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label class="block text-sm text-gray-700 mb-1">Assigned Employees</label>
                                        <div class="bg-gray-50 border border-gray-300 rounded-lg p-2 max-h-40 overflow-y-auto">
                                            <?php foreach ($employees as $employee): ?>
                                            <div class="flex items-center mb-1">
                                                <input 
                                                    type="checkbox" 
                                                    name="services[<?php echo $service_counter; ?>][employee_ids][]" 
                                                    value="<?php echo $employee['id']; ?>" 
                                                    class="mr-2 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                                    <?php echo in_array($employee['id'], $employee_ids) ? 'checked' : ''; ?>
                                                    >
                                                <span class="text-sm text-gray-700"><?php echo htmlspecialchars($employee['name']); ?></span>
                                            </div>
                                            <?php endforeach; ?>
                                        </div>
                                    </div>
                    ...
                    <!-- In the view invoice modal, update the summary section: -->
                    <div class="ml-auto w-full md:w-1/2 space-y-3 mt-6">
                        ...
                        <?php if ($invoice['discount_type'] !== 'none'): ?>
                        <div class="flex justify-between">
                            <span class="text-sm text-gray-600">Discount (<?php 
                                echo $invoice['discount_type'] === 'flat' ? 'Flat ₹' . number_format($invoice['overall_discount_value'], 2) : 
                                number_format($invoice['overall_discount_value'], 2) . '%'; ?>):</span>
                            <span class="text-red-600">- ₹<?php echo number_format($invoice['total_amount'] - $invoice['discounted_amount'], 2); ?></span>
                        </div>
                        <?php endif; ?>
        ...
        <div class="relative bg-white rounded-lg shadow-lg border border-gray-200">
            ...
            <div class="p-4 md:p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-2 bg-gray-50 p-3 rounded-lg">
                        ...
                        <div class="grid grid-cols-2 gap-2 text-sm">
                            ...
                            <span class="text-gray-900"><?php echo date('j M Y', strtotime($invoice['invoice_date'])); ?></span>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-pink-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-pink-700 uppercase tracking-wider">Stylist(s)</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <?php echo htmlspecialchars($item['employee_names']); ?>
                                </td>
                ...
                <div class="ml-auto w-full md:w-1/2 space-y-3 mt-6">
                    ...
                    <div class="flex justify-between">
                        ...
                        Discount (<?php 
                            echo $invoice['discount_type'] === 'flat' 
                                ? 'Flat ₹' . number_format($invoice['overall_discount_value'], 2) 
                                : number_format($invoice['overall_discount_value'], 2) . '%'; 
                            ?>):
                        </span>
                        <span class="text-red-600">- ₹<?php echo number_format($invoice['total_amount'] - $invoice['discounted_amount'], 2); ?></span>
                    </div>
            ...
            <div class="flex items-center justify-between p-4 md:p-6 border-t border-gray-200 rounded-b bg-gray-50">
                <div class="text-sm text-gray-500">
                    <p>Invoice generated on <?php echo date('j M Y \a\t g:i A', strtotime($invoice['invoice_date'])); ?></p>
...
<script>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
       ...
       <div>
          ...
          <div class="bg-gray-50 border border-gray-300 rounded-lg p-2 max-h-40 overflow-y-auto">
             ...
             <div class="flex items-center mb-1">
                <input 
                   type="checkbox" 
                   name="services[${counter}][employee_ids][]" 
                   value="<?php echo $employee['id']; ?>" 
                   class="mr-2 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                   >
                <span class="text-sm text-gray-700"><?php echo htmlspecialchars($employee['name']); ?></span>
       ...
       <div>
          <label class="block text-sm text-gray-700 mb-1">Price (₹)</label>
          <input type="number" min="0" step="0.01" name="services[${counter}][price]" class="service-price bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5">
       </div>
       <div>
          <label class="block text-sm text-gray-700 mb-1">Qty</label>
    ...
    <div class="bg-gray-50 border border-gray-300 rounded-lg p-2 max-h-40 overflow-y-auto">
       ...
       <div class="flex items-center mb-1">
          <input 
             type="checkbox" 
             name="products[${counter}][employee_ids][]" 
             value="<?php echo $employee['id']; ?>" 
             class="mr-2 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
             >
          <span class="text-sm text-gray-700"><?php echo htmlspecialchars($employee['name']); ?></span>
       ...
       <?php endforeach; ?>
                    ...
                    <div>
                        <label class="block text-sm text-gray-700 mb-1">Price (₹)</label>
                        <input type="number" min="0" step="0.01" name="products[${counter}][price]" class="product-price bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5">
                    </div>
                    <div>
                        <label class="block text-sm text-gray-700 mb-1">Qty</label>
    ...
    document.addEventListener('change', function(e) {
     if (e.target.classList.contains('service-select')) {
         ...
         // Immediately calculate total when price changes
    ...
    document.getElementById('customer_search').addEventListener('input', function() {
     ...
     const customerIdField = document.getElementById('customer_id_hidden');
     
     // Find matching option
     const selectedOption = document.querySelector(`#customer_options option[value="${searchValue}"]`);
     
     if (selectedOption) {
         // Customer exists - set the hidden ID field and hide add button
         customerIdField.value = selectedOption.getAttribute('data-id');
         addCustomerBtn.classList.add('hidden');
     } else {
         // Customer doesn't exist - clear ID and show add button (if search not empty)
         customerIdField.value = '';
         addCustomerBtn.classList.toggle('hidden', searchValue === '');
     }
    });
    
    // Clear add button when input is empty
    document.getElementById('customer_search').addEventListener('blur', function() {
     if (this.value.trim() === '') {
         document.getElementById('addCustomerBtn').classList.add('hidden');
     }
    });
